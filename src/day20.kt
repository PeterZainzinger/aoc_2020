...
data class Tile(val id: Int, val pixels: List<List<Pixel>>) {
    override fun toString() =
        "\nTile $id:\n" + pixels.map { it.joinToString(separator = "") { it.item.toString() } }
            .joinToString("\n") + "\n\n"
    ...
    fun allOptions(): List<Tile> {
        val rot0 = this
        val rot1 = rotate()
        val rot2 = rot1.rotate()
        val rot3 = rot2.rotate()
        return listOf(rot0, rot1, rot2, rot3).flatMap { source ->
...
fun day20() {
    ...
    tiles.indices.forEach { loop ->
        println("current loop $loop")
        currentStates = currentStates.flatMap { searchForNext(it) ?: emptyList() }
    }
    ...
    println(monsterPattern)
    println(monsterIndices)
